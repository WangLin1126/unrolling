#!/usr/bin/env python3
"""Training script for PAP (Plug-and-Play) heterogeneous denoiser chain.

Similar to train.py but supports heterogeneous denoiser configurations
where each stage can have a different denoiser architecture.

Usage:
    torchrun --standalone --nproc_per_node=2 pap/pap_train.py --config configs/pap_example.yaml
"""
...
from pap.config import parse_denoiser_chain, validate_chain_config
from pap.utils import get_trainable_params, summarize_chain
...
def resolve_checkpoint_path(ckpt_cfg):
    if not ckpt_cfg:
        return None
...
def setup_logger(log_file: Path, rank: int) -> logging.Logger:
    logger = logging.getLogger(f"pap_train_rank{rank}")
...
def save_checkpoint(path, model, criterion, optimizer, scheduler,
                    epoch, best_psnr, best_val_loss, no_improve_count, cfg, extra=None):
...
def load_checkpoint(ckpt_path, model, criterion, optimizer=None, scheduler=None,
    ...
    return {"epoch": 0, "best_psnr": 0.0, "best_val_loss": float("inf"),
            "no_improve_count": 0, "legacy_state_dict_only": True}
...
def build_exp_dir(cfg: dict, base: str = "results") -> Path:
    ...
    chain = cfg.get("denoiser_chain", [])
    T = len(chain)
    chain_desc = "-".join(e.get("type", "?") for e in chain)
...
def main():
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("Starting PAP training")
        ...
        if use_ddp:
            ...
            if criterion_has_params:
                criterion = DDP(criterion, device_ids=[local_rank],
                                output_device=local_rank, broadcast_buffers=False)

        # Only optimize trainable params
        trainable_params = get_trainable_params(unwrap_model(model))
        all_params = trainable_params + list(criterion.parameters())
        ...
        if resume_ckpt is not None:
            ckpt = load_checkpoint(
                ckpt_path=resume_ckpt, model=model, criterion=criterion,
        ...
        for epoch in range(start_epoch, tc["epochs"] + 1):
            ...
            for step, (blur, sharp, blur_sigmas, noise_sigmas, targets) in enumerate(train_loader, 1):
                ...
                if use_precomputed:
                    ...
                    result = model(blur=blur, blur_sigma=blur_sigmas,
            ...
            if is_main_process():
                logger.info(
                    ...
                    f"train_loss={avg_loss:.5f} lr={current_lr:.2e} time={elapsed:.1f}s"
                )
                save_checkpoint(
                    train_dir / "last.pth", model=model, criterion=criterion,
                    optimizer=optimizer, scheduler=scheduler, epoch=epoch,
                    best_psnr=best_psnr, best_val_loss=best_val_loss,
                    no_improve_count=no_improve_count, cfg=cfg, extra={"tag": "last"},
                )
            ...
            if epoch % tc["val_every"] == 0:
                ...
                if is_main_process():
                    ...
                    if avg_psnr > best_psnr:
                        best_psnr = avg_psnr
                        save_checkpoint(
                            train_dir / "best.pth", model=model, criterion=criterion,
                            optimizer=optimizer, scheduler=scheduler, epoch=epoch,
                            best_psnr=best_psnr, best_val_loss=best_val_loss,
                            no_improve_count=no_improve_count, cfg=cfg,
                            extra={"tag": "best_by_psnr"},
                        )
                        logger.info(f"Saved best checkpoint (PSNR={best_psnr:.2f})")
                    ...
                    if epoch % 10 == 0:
                        save_checkpoint(
                            train_dir / f"ckpt_e{epoch}.pth", model=model,
                    ...
                    if patience > 0 and no_improve_count >= patience:
                        ...
                        save_checkpoint(
                            train_dir / "ckpt_early_stop.pth", model=model,
                            criterion=criterion, optimizer=optimizer,
                            scheduler=scheduler, epoch=epoch,
                            best_psnr=best_psnr, best_val_loss=best_val_loss,
                            no_improve_count=no_improve_count, cfg=cfg,
        ...
        if should_run_test:
            if best_ckpt.exists():
                ...
                logger.info("=" * 50)
                from pap.pap_evaluate import run_pap_evaluate
                run_pap_evaluate(cfg, str(best_ckpt), str(test_dir))
            else:
                logger.warning("Skip test: best.pth does not exist.")
        ...
        if logger is None:
            ...
            try:
                if is_main_process() and "train_dir" in locals():
                    save_checkpoint(
                        train_dir / "crash.pth",
                        model=locals()["model"], criterion=locals()["criterion"],
                        ...
                        cfg=locals().get("cfg", {}),
                        extra={"tag": "crash", "exception": repr(e)},
                    )
            except Exception:
